...
flow = mdp.Flow([ica, artifacts, bandpass,
                 embed, csp, var, fda,
                 gaussian, lowpass, cutoff], verbose=0)
...
gridsearch_parameters = {
    # ridge: {'ridge_param': [0.001, 0.01, 0.1, 0.5, 1, 2, 3, 10, 100]}
    # var: {'box_width': np.arange(50, 200, 50)},
    embed: {'time_frames': np.append(1, np.arange(5, 20, 5)),
            'gap': np.append(1, np.arange(2, 20, 4))},
#    fda: {'output_dim': np.arange(1, 4, 1)},
    # lowpass: {'Wn': np.arange(0.001, 0.01, 0.001)}
}

def pearson_corr(sig, target):
    return -1 * pearsonr(sig[:, 0], target[:, 0])[0]

def autocorr(sig, target):
    return -1 * np.correlate(sig[:, 0], target[:, 0])[0]

def accuracy(sig, target):
    return np.mean(np.sign(sig) == target)
# # opt = Oger.evaluation.Optimizer(gridsearch_parameters, Oger.utils.auc_error)
# opt = Oger.evaluation.Optimizer(gridsearch_parameters, pearson_corr)
...
yy = np.hstack(y_split)
# print(Oger.utils.nrmse(out[ignore:], y[ignore:]))
# print(accuracy(out[ignore:], y[ignore:]))

print(Oger.utils.nrmse(out[:, 0], yy))
print(accuracy(out[:, 0], yy))
...
for data_ix, test_ix in zip(data_ixs, test_ixs):
    ...
    print(pearson_corr(p, y_split_newaxis[tix]))
    print(np.mean(np.sign(p) == y_split_newaxis))
...
print(np.mean(np.sign(pred) == np.hstack(y_split)))
